<html lang="en">

<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta http-equiv="Content-Language" content="en">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    <title>Aplikasi APM - Badilag</title>
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, shrink-to-fit=no">

    <!-- Disable tap highlight on IE -->
    <meta name="msapplication-tap-highlight" content="no">

    <link rel="stylesheet" href="css/base.min.css">
    <script type="text/javascript" src="https://pmpapm.badilag.net/apm/assets/js/core/libraries/jquery.min.js"></script>
    <style>
        .widget-content .widget-content-left .widget-heading {
            opacity: 1;
            font-weight: 700;
        }

        .app-header.header-text-light .app-header-right>.header-btn-lg .widget-heading {
            color: white;
        }

        .table.dataTable thead>tr>td.sorting,
        table.dataTable thead>tr>td.sorting_asc,
        table.dataTable thead>tr>td.sorting_desc,
        table.dataTable thead>tr>th.sorting,
        table.dataTable thead>tr>th.sorting_asc,
        table.dataTable thead>tr>th.sorting_desc {
            background-image: linear-gradient(to top, #0ba360 0, #3cba92 100%) !important;
            color: white;
            text-align: center;
        }

        .btn-primary {
            color: #fff;
            background-image: linear-gradient(to top, #0ba360 0, #3cba92 100%) !important;
            border-color: white !important;
        }
    </style>
    0
    <style type="text/css">

    </style>
    <style type="text/css">

    </style>
    <style type="text/css">

    </style>
    <style type="text/css">
        @font-face {
            font-family: 'Didact Gothic';
            src: url(data:font/woff2;base64,) format("woff");
            font-weight: normal;
            font-style: normal
        }
    </style>
</head>

<body>
    <div class="app-container app-theme-white body-tabs-shadow fixed-header">
        <!--Header START-->
        <div class="app-header header-shadow bg-grow-early header-text-light">
            <div class="app-header__logo">
                <img src="https://pmpapm.badilag.net/apm/assets/images/logo_ma.png" width="15%" alt="">
                <span style="font-size: 20px; color: white; padding-left: 10px;"> PMP-APM</span>
            </div>
            <div class="app-header__mobile-menu">
                <div>
                    <button type="button" class="hamburger hamburger--elastic mobile-toggle-nav">
                        <span class="hamburger-box">
                            <span class="hamburger-inner"></span>
                        </span>
                    </button>
                </div>
            </div>
            <div class="app-header__menu">
                <span>
                    <button type="button" class="btn-icon btn-icon-only btn btn-primary btn-sm mobile-toggle-header-nav">
                        <span class="btn-icon-wrapper">
                            <i class="fa fa-ellipsis-v fa-w-6"></i>
                        </span>
                    </button>
                </span>
            </div>
            <div class="app-header__content">
                <div class="app-header-left">
                    <a href="https://pmpapm.badilag.net/apm/" class="text-white"> <i class="fa fa-home"> </i> Beranda</a>
                </div>
                <div class="app-header-right">
                    <div class="header-btn-lg pr-0">
                        <div class="widget-content p-0">
                            <div class="widget-content-wrapper">
                                <div class="widget-content-left">
                                    <div class="btn-group">
                                        <a data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="p-0 btn">
                                            <img width="42" class="ml-2 rounded-circle" src="https://pmpapm.badilag.net/apm/assets/images/avatar.jpg" alt="">
                                            <i class="fa fa-angle-down ml-2 opacity-8"></i>

                                        </a>
                                        <div tabindex="-1" role="menu" aria-hidden="true" class="rm-pointers dropdown-menu-lg dropdown-menu dropdown-menu-right">

                                            <div class="scroll-area-xs" style="height: 100px;">
                                                <div class="scrollbar-container ps">
                                                    <ul class="nav flex-column">
                                                        <li class="nav-item-header nav-item">My Account
                                                        </li>
                                                        <li class="nav-item">
                                                            <a href="https://pmpapm.badilag.net/apm/page/setting" class="nav-link">
                                                                Setting
                                                            </a>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </div>
                                            <ul class="nav flex-column">
                                                <li class="nav-item-divider mb-0 nav-item"></li>
                                            </ul>
                                            <ul class="nav flex-column">
                                                <li class="nav-item-btn text-center nav-item">
                                                    <a href="https://pmpapm.badilag.net/apm/main/logout" class="btn-wide btn-primary btn-shine btn btn-focus">Keluar
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                                <div class="widget-content-left  ml-3 header-user-info">
                                    <div class="widget-heading">
                                        PENGADILAN AGAMA NGAWI </div>
                                    <div class="widget-subheading">
                                        401508 </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!--Header END-->
        <div class="app-main">
            <div class="app-main__outer">
                <div class="app-main__inner">
                    <script>
                        $(document).ready(function() {

                            $(document).on('click', '.lke-baru', function(e) {
                                $('#exampleModal').modal('show');
                                $('.form-add-laporan').attr('data-id', '');
                                $('.form-add-laporan').trigger('reset');
                            });

                            $("body").on('click', '.edit-lke', function() {
                                var idRow = $(this).attr('data-id');
                                $('.form-add-laporan').attr('data-id', idRow);
                                $.getJSON("https://pmpapm.badilag.net/apm/api/get_single_lke//" + idRow, function(json) {
                                    $('.form-add-laporan input[name=tahun]').val(json.tahun);
                                    $('.form-add-laporan select[name=jenis]').val(json.lke_tahapan);
                                    $('#exampleModal').modal('show');
                                }).error(function() {
                                    alert("error");
                                });
                            });

                            $(document).on('submit', '.form-add-laporan', function(e) {
                                e.preventDefault();
                                var idRow = $(this).attr('data-id');
                                $.ajax({
                                    type: "POST",
                                    url: "https://pmpapm.badilag.net/apm/main/save_lke/" + idRow,
                                    data: new FormData(this),
                                    dataType: 'json',
                                    processData: false,
                                    contentType: false,
                                    success: function(response) {
                                        tabel.ajax.reload(null, false);
                                        toastr.success('Berhasil Disimpan');
                                        $('#exampleModal').modal('hide');
                                    },
                                    error: function(xhr) {
                                        toastr.error(xhr.responseJSON.message);
                                    }
                                });
                            });

                            $(document).on('click', '.hapus-lke', function(e) {
                                e.preventDefault();
                                var idRow = $(this).attr("data-id");
                                hapus_lke("Apakah anda yakin akan menghapus LKE ini ?", idRow, 1);
                            });

                            function hapus_lke(message, idRow, step) {
                                $('.form-confirm').attr("data-id", 'hapuslke');
                                $('.modal-confirm-message').html(message);
                                $('.modal-confirm-body').html("");
                                $('.modal-confirm-btnNo').html("Tidak");
                                $('.modal-confirm-btnYes').html("Ya");
                                $('#modal_confirm').modal('show');

                                $(document).on('submit', '.form-confirm[data-id=hapuslke]', function(e) {
                                    e.preventDefault();
                                    $.ajax({
                                        type: "GET",
                                        url: "https://pmpapm.badilag.net/apm/main/hapus_lke/" + idRow + '/' + step,
                                        cache: false,
                                        success: function(json) {
                                            if (json.ask_again) {
                                                hapus_lke(json.message, json.id_lke, json.step);
                                            } else {
                                                toastr.success('LKE Dihapus');
                                                $('#modal_confirm').modal('hide');
                                                tabel.ajax.reload(null, false);
                                            }
                                        },
                                        error: function(xhr) {
                                            toastr.danger('Error');
                                        }
                                    });
                                });
                            }

                            var tabel = $('#tabelRiwayatLKE').DataTable({
                                ajax: 'https://pmpapm.badilag.net/apm/api/get_data_dashboard/',
                                autoWidth: false,
                                order: [0, 'asc'],
                                "rowCallback": function(row, data, iDisplayIndex) {
                                    var index = iDisplayIndex + 1;
                                    $('td:eq(0)', row).html(index);
                                    return row;
                                },
                                "columns": [{
                                        data: 'id_lke'
                                    },
                                    {
                                        data: 'nama'
                                    },
                                    {
                                        data: 'tahun'
                                    },
                                    {
                                        data: 'lke_tahapan',
                                        "render": function(data, type, col, meta) {
                                            var jns = ["", "Penilaian Awal/ Ulang", "Surveilance 1", "Surveilance 2", "Surveilance 3"];
                                            return jns[data];
                                        }
                                    },
                                    {
                                        data: 'id_lke',
                                        render: function(data, type, col, meta) {
                                            return '<a href="https://pmpapm.badilag.net/apm/lke/form_a/' + data + '" class="mb-2 mr-2 btn-icon btn-pill btn btn-warning"><i class="pe-7s-note2 btn-icon-wrapper"> </i> LKE Telusur Dokumen</a>' +
                                                '<a href="https://pmpapm.badilag.net/apm/lke/form_b/' + data + '" class="mb-2 mr-2 btn-icon btn-pill btn btn-warning"><i class="pe-7s-note2 btn-icon-wrapper"> </i> LKE Observasi Implementasi</a>';
                                        }
                                    },

                                    {
                                        data: 'nilai_3',
                                        render: function(data, type, col, meta) {
                                            return col.nilai_3;
                                        }
                                    },
                                    {
                                        data: "id_lke",
                                        "render": function(data, type, col, meta) {
                                            var btnDownload = "";
                                            // if(col.file_dokumen == null){
                                            //     btnDownload = '<button class="btn btn-warning disabled">Download</button>';
                                            // }else{
                                            //     btnDownload = '<a href="https://pmpapm.badilag.net/apm/lke/download_dokumen_penilaian/'+col.file_dokumen+'" class="btn btn-warning ">Download</a>';
                                            // }
                                            return btnDownload + ' <button data-id=' + col.id_lke + ' class="btn btn-warning edit-lke">Edit</button> <button data-id=' + col.id_lke + ' class="btn btn-danger hapus-lke">Hapus</button>';
                                        }
                                    }
                                ],
                                language: {
                                    search: '<span>Filter:</span> _INPUT_',
                                    searchPlaceholder: 'Type to filter...',
                                    lengthMenu: '<span>Show:</span> _MENU_',
                                    paginate: {
                                        'first': 'Awal',
                                        'last': 'Akhir',
                                        'next': '&rarr;',
                                        'previous': '&larr;'
                                    }
                                },
                                preDrawCallback: function() {
                                    $(this).find('tbody tr').slice(-3).find('.dropdown, .btn-group').removeClass('dropup');
                                }
                            });


                        });
                    </script>

                    <div class="tabs-animation">

                        <div class="card mb-3">
                            <div class="card-header-tab card-header">
                                <div class="card-header-title font-size-lg text-capitalize font-weight-normal"><i class="header-icon lnr-laptop-phone mr-3 text-muted opacity-6"> </i>Daftar Penilaian -
                                    PENGADILAN AGAMA NGAWI</div>
                                <div class="btn-actions-pane-right actions-icon-btn">
                                    <button type="button" class="btn mr-2 mb-2 btn-primary lke-baru">
                                        Laporan Baru
                                    </button>
                                </div>
                            </div>
                            <div class="card-body">
                                <div id="tabelRiwayatLKE_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                                    <div class="row">
                                        <div class="col-sm-12 col-md-6">
                                            <div class="dataTables_length" id="tabelRiwayatLKE_length"><label><span>Show:</span> <select name="tabelRiwayatLKE_length" aria-controls="tabelRiwayatLKE" class="custom-select custom-select-sm form-control form-control-sm">
                                                        <option value="10">10</option>
                                                        <option value="25">25</option>
                                                        <option value="50">50</option>
                                                        <option value="100">100</option>
                                                    </select></label></div>
                                        </div>
                                        <div class="col-sm-12 col-md-6">
                                            <div id="tabelRiwayatLKE_filter" class="dataTables_filter"><label><span>Filter:</span> <input type="search" class="form-control form-control-sm" placeholder="Type to filter..." aria-controls="tabelRiwayatLKE"></label></div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <table style="width: 100%;" class="table table-hover table-striped table-bordered dataTable no-footer" id="tabelRiwayatLKE" role="grid" aria-describedby="tabelRiwayatLKE_info">
                                                <thead>
                                                    <tr role="row">
                                                        <th class="sorting_asc" tabindex="0" aria-controls="tabelRiwayatLKE" rowspan="1" colspan="1" aria-label="No: activate to sort column descending" aria-sort="ascending">No</th>
                                                        <th class="sorting" tabindex="0" aria-controls="tabelRiwayatLKE" rowspan="1" colspan="1" aria-label="Satker: activate to sort column ascending">Satker</th>
                                                        <th class="sorting" tabindex="0" aria-controls="tabelRiwayatLKE" rowspan="1" colspan="1" aria-label="Tahun: activate to sort column ascending">Tahun</th>
                                                        <th class="sorting" tabindex="0" aria-controls="tabelRiwayatLKE" rowspan="1" colspan="1" aria-label="Tahapan: activate to sort column ascending">Tahapan</th>
                                                        <th class="sorting" tabindex="0" aria-controls="tabelRiwayatLKE" rowspan="1" colspan="1" aria-label="LKE: activate to sort column ascending">LKE</th>
                                                        <th class="sorting" tabindex="0" aria-controls="tabelRiwayatLKE" rowspan="1" colspan="1" aria-label="Nilai Akhir: activate to sort column ascending">Nilai Akhir</th>
                                                        <th class="sorting" tabindex="0" aria-controls="tabelRiwayatLKE" rowspan="1" colspan="1" aria-label="#: activate to sort column ascending">#</th>
                                                    </tr>
                                                </thead>
                                                <tbody>


                                                    <tr role="row" class="odd">
                                                        <td class="sorting_1">1</td>
                                                        <td>PENGADILAN AGAMA NGAWI</td>
                                                        <td>2021</td>
                                                        <td>Surveilance 3</td>
                                                        <td><a href="https://pmpapm.badilag.net/apm/lke/form_a/1093" class="mb-2 mr-2 btn-icon btn-pill btn btn-warning"><i class="pe-7s-note2 btn-icon-wrapper"> </i> LKE Telusur Dokumen</a><a href="https://pmpapm.badilag.net/apm/lke/form_b/1093" class="mb-2 mr-2 btn-icon btn-pill btn btn-warning"><i class="pe-7s-note2 btn-icon-wrapper"> </i> LKE Observasi Implementasi</a></td>
                                                        <td></td>
                                                        <td> <button data-id="1093" class="btn btn-warning edit-lke">Edit</button> <button data-id="1093" class="btn btn-danger hapus-lke">Hapus</button></td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-sm-12 col-md-5">
                                            <div class="dataTables_info" id="tabelRiwayatLKE_info" role="status" aria-live="polite">Showing 1 to 1 of 1 entries</div>
                                        </div>
                                        <div class="col-sm-12 col-md-7">
                                            <div class="dataTables_paginate paging_simple_numbers" id="tabelRiwayatLKE_paginate">
                                                <ul class="pagination">
                                                    <li class="paginate_button page-item previous disabled" id="tabelRiwayatLKE_previous"><a href="#" aria-controls="tabelRiwayatLKE" data-dt-idx="0" tabindex="0" class="page-link">←</a></li>
                                                    <li class="paginate_button page-item active"><a href="#" aria-controls="tabelRiwayatLKE" data-dt-idx="1" tabindex="0" class="page-link">1</a></li>
                                                    <li class="paginate_button page-item next disabled" id="tabelRiwayatLKE_next"><a href="#" aria-controls="tabelRiwayatLKE" data-dt-idx="2" tabindex="0" class="page-link">→</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>


                </div>
                <div class="app-wrapper-footer">
                    <div class="app-footer">
                        <div class="app-footer__inner">
                            <div class="app-footer-left pl-4">
                                Hak Cipta Badan Peradilan Agama - Mahkamah Agung RI
                            </div>
                            <div class="app-footer-right">
                                <ul class="header-megamenu nav">

                                    <li class="nav-item">
                                        <a data-placement="top" rel="popover-focus" data-offset="300" data-toggle="popover-custom" class="nav-link" data-original-title="" title="">
                                            Grid Menu
                                            <div class="badge badge-dark ml-0 ml-1">
                                                <small>NEW</small>
                                            </div>
                                            <i class="fa fa-angle-up ml-2 opacity-8"></i>
                                        </a>
                                        <div class="rm-max-width rm-pointers">
                                            <div class="d-none popover-custom-content">
                                                <div class="dropdown-menu-header">
                                                    <div class="dropdown-menu-header-inner bg-tempting-azure">
                                                        <div class="menu-header-image opacity-1" style="background-image: url('https://pmpapm.badilag.net/apm//assets/images/dropdown-header/city5.jpg');">
                                                        </div>
                                                        <div class="menu-header-content text-dark">
                                                            <h5 class="menu-header-title">Two Column Grid</h5>
                                                            <h6 class="menu-header-subtitle">Easy grid navigation inside
                                                                popovers</h6>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="grid-menu grid-menu-2col">
                                                    <div class="no-gutters row">
                                                        <div class="col-sm-6">
                                                            <button class="btn-icon-vertical btn-transition-text btn-transition btn-transition-alt pt-2 pb-2 btn btn-outline-dark"><i class="lnr-lighter text-dark opacity-7 btn-icon-wrapper mb-2">
                                                                </i>Automation
                                                            </button>
                                                        </div>
                                                        <div class="col-sm-6">
                                                            <button class="btn-icon-vertical btn-transition-text btn-transition btn-transition-alt pt-2 pb-2 btn btn-outline-danger"><i class="lnr-construction text-danger opacity-7 btn-icon-wrapper mb-2">
                                                                </i>Reports
                                                            </button>
                                                        </div>
                                                        <div class="col-sm-6">
                                                            <button class="btn-icon-vertical btn-transition-text btn-transition btn-transition-alt pt-2 pb-2 btn btn-outline-success"><i class="lnr-bus text-success opacity-7 btn-icon-wrapper mb-2">
                                                                </i>Activity
                                                            </button>
                                                        </div>
                                                        <div class="col-sm-6">
                                                            <button class="btn-icon-vertical btn-transition-text btn-transition btn-transition-alt pt-2 pb-2 btn btn-outline-focus"><i class="lnr-gift text-focus opacity-7 btn-icon-wrapper mb-2">
                                                                </i>Settings
                                                            </button>
                                                        </div>
                                                    </div>
                                                </div>
                                                <ul class="nav flex-column">
                                                    <li class="nav-item-divider nav-item"></li>
                                                    <li class="nav-item-btn clearfix nav-item">
                                                        <div class="float-left">
                                                            <button class="btn btn-link btn-sm">Link Button</button>
                                                        </div>
                                                        <div class="float-right">
                                                            <button class="btn-shadow btn btn-info btn-sm">Info
                                                                Button</button>
                                                        </div>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <button class="btn btn-success" onclick="topFunction()" id="myBtn" title="Go to top">Kembali ke atas</button>
    <style>
        #myBtn {
            display: none;
            /* Hidden by default */
            position: fixed;
            /* Fixed/sticky position */
            bottom: 20px;
            /* Place the button at the bottom of the page */
            right: 30px;
            /* Place the button 30px from the right */
            z-index: 99;
            /* Make sure it does not overlap */

            cursor: pointer;
            /* Add a mouse pointer on hover */

        }

        #myBtn:hover {
            background-color: #555;
            /* Add a dark-grey background on hover */
        }
    </style>
    <script>
        mybutton = document.getElementById("myBtn");

        // When the user scrolls down 20px from the top of the document, show the button
        window.onscroll = function() {
            scrollFunction()
        };

        function scrollFunction() {
            if (document.body.scrollTop > 20 || document.documentElement.scrollTop > 20) {
                mybutton.style.display = "block";
            } else {
                mybutton.style.display = "none";
            }
        }

        // When the user clicks on the button, scroll to the top of the document
        function topFunction() {
            document.body.scrollTop = 0; // For Safari
            document.documentElement.scrollTop = 0; // For Chrome, Firefox, IE and Opera
        }
    </script>

    <!--CORE-->

    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.2.1/js/bootstrap.bundle.min.js" crossorigin="anonymous">
    </script>
    <script src="https://cdn.jsdelivr.net/npm/metismenu"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/app.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/demo.js"></script>


    <!--Chart.js-->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.4.0/Chart.min.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/charts/chartsjs-utils.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/charts/chartjs.js"></script>

    <!--FORMS-->

    <!--Clipboard-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/clipboard.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/clipboard.js"></script>

    <!--Datepickers-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/datepicker.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/daterangepicker.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/moment.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/datepicker.js"></script>

    <!--Multiselect-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/bootstrap-multiselect.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.6-rc.0/js/select2.min.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/input-select.js"></script>

    <!--Form Validation-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/form-validation.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/form-validation.js"></script>

    <!--Form Wizard-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/form-wizard.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/form-wizard.js"></script>

    <!--Input Mask-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/input-mask.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/input-mask.js"></script>

    <!--RangeSlider-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/wnumb.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/range-slider.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/range-slider.js"></script>

    <!--Textarea Autosize-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/textarea-autosize.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/form-components/textarea-autosize.js"></script>

    <!--Toggle Switch -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/form-components/toggle-switch.js"></script>

    <!--BlockUI -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/blockui.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/blockui.js"></script>

    <!--Calendar -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/calendar.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/calendar.js"></script>

    <!--Slick Carousel -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/carousel-slider.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/carousel-slider.js"></script>

    <!--Circle Progress -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/circle-progress.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/circle-progress.js"></script>

    <!--CountUp -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/count-up.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/count-up.js"></script>

    <!--Cropper -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/cropper.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/jquery-cropper.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/image-crop.js"></script>

    <!--Maps -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/gmaps.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/jvectormap.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/maps-word-map.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/maps.js"></script>

    <!--Guided Tours -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/guided-tours.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/guided-tours.js"></script>

    <!--Ladda Loading Buttons -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/ladda-loading.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/spin.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/ladda-loading.js"></script>

    <!--Rating -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/rating.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/rating.js"></script>

    <!--Perfect Scrollbar -->
    <!-- <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/scrollbar.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/scrollbar.js"></script> -->

    <!--Toastr-->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.js" crossorigin="anonymous"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/toastr.js"></script>

    <!--SweetAlert2-->
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@8"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/sweet-alerts.js"></script>

    <!--Tree View -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/treeview.js"></script>
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/treeview.js"></script>

    <!--pnotify -->
    <script src="https://pmpapm.badilag.net/apm//assets/js/plugins/notifications/pnotify.min.js"></script>


    <!--TABLES -->
    <!--DataTables-->
    <script src="https://cdn.datatables.net/1.10.19/js/jquery.dataTables.min.js" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/datatables.net-bs4@1.10.19/js/dataTables.bootstrap4.min.js" crossorigin="anonymous"></script>
    <script src="https://cdn.datatables.net/responsive/2.2.3/js/dataTables.responsive.min.js" crossorigin="anonymous">
    </script>
    <script src="https://cdn.datatables.net/responsive/2.2.3/js/responsive.bootstrap.min.js" crossorigin="anonymous">
    </script>

    <!--Bootstrap Tables-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/vendors/tables.js"></script>

    <!--Tables Init-->
    <script src="https://pmpapm.badilag.net/apm//assets/js/scripts-init/tables.js"></script>

    <!-- modal lke -->
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Laporan Baru</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-add-laporan" data-id="">
                    <div class="modal-body">

                        <div class="position-relative form-group"><label for="exampleEmail" class="">Tahun</label>
                            <input name="tahun" placeholder="Masukkan tahun" type="number" class="form-control">
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Jenis Penilaian</label>
                            <select name="jenis" class="form-control">
                                <option value="">Pilih</option>
                                <option value="1">Penilaian Awal/ Ulang</option>
                                <option value="2">Surveilance 1</option>
                                <option value="3">Surveilance 2</option>
                                <option value="4">Surveilance 3</option>
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
    </div>



    <!-- modal upload eviden -->
    <div class="modal fade" id="modalUploadEviden" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Upload Eviden</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-upload-eviden" data-msp="" data-nilai="">
                    <div class="modal-body">
                        <div class="position-relative form-group">
                            <label for="exampleEmail" class="">Jenis</label>
                            <select name="jenis" class="form-control"></select>
                        </div>
                        <div class="position-relative form-group">
                            <label for="exampleSelect" class="">File</label>
                            <input type="file" name="file" class="form-control">
                            <span class="help-block">*file yang didukung : png,jpg,jpeg,doc,docx,png,xls,xlsx</span>
                        </div>
                        <div class="position-relative form-group">
                            <label for="exampleSelect" class="">Keterangan</label>
                            <input type="text" name="keterangan" class="form-control">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- modal link eviden -->
    <div class="modal fade" id="modalLinkEviden" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Link Download Eviden</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-link-eviden" data-msp="" data-nilai="">
                    <div class="modal-body">
                        <div class="position-relative form-group"><label for="exampleEmail" class="">Jenis</label>
                            <select name="jenis" class="form-control"></select>
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Link</label>
                            <input type="text" name="link" class="form-control">
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Keterangan</label>
                            <input type="text" name="keterangan" class="form-control">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Save changes</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- modal buka eviden -->
    <div class="modal fade" id="modalOpenEviden" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Eviden</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-link-eviden" data-msp="" data-nilai="">
                    <div class="modal-body">
                        <p>
                        </p>
                        <h5 class="keterangan-eviden"></h5>
                        <p></p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="button" class="btn btn-danger arsipkan-eviden">Hapus Tautan</button>
                        <a type="button" class="btn btn-primary get-eviden">Download</a>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- modal asesor -->
    <div class="modal fade" id="modalAsesor" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Form Asesor</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-asesor" data-id="">
                    <div class="modal-body">
                        <div class="position-relative form-group"><label for="exampleSelect" class="">NIP</label>
                            <input type="text" name="nip" class="form-control">
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Nama</label>
                            <input type="text" name="nama" class="form-control">
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Jabatan</label>
                            <select name="jabatan" class="form-control">
                                <option value="Lead Asesor">Lead Asesor</option>
                                <option value="Pendamping Asesor">Pendamping Asesor</option>
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                        <button type="button" class="btn btn-danger btn-hapus-asesor" data-id="">Hapus Asesor</button>
                        <button type="submit" class="btn btn-primary">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- modal ganti password -->
    <div class="modal fade" id="modal_ganti_pass" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Ganti Password Anda</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-ganti-pass" data-id="">
                    <div class="modal-body">
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Password Lama</label>
                            <input type="password" name="password_lama" class="form-control">
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Password Baru</label>
                            <input type="password" name="password" class="form-control">
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Ulangi Password</label>
                            <input type="password" name="ulangi_password" class="form-control">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <!-- <button type="button" class="btn btn-secondary" data-dismiss="modal">Nanti</button> -->
                        <button type="submit" class="btn btn-primary">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- modal upload dokumen lke -->
    <div class="modal fade" id="modal_upload_dokumen_penilaian" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-xl" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Upload Dokumen Penilaian</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-upload-DokumenPenilaian" data-msp="" data-nilai="">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg-4">
                                <div class="position-relative form-group">
                                    <label for="exampleSelect" class="">File</label>
                                    <div>
                                        <input type="file" name="file">
                                    </div>

                                    <span class="help-block">*file yang didukung : pdf (max. 5MB)</span>
                                </div>
                            </div>
                            <div class="col-lg-8">

                                <div class="card">
                                    <div class="card-body">
                                        Dokumen yang diunggah: <br>
                                        <ol>
                                            <li>Laporan </li>
                                            <li>Berita Acara </li>
                                            <li>Form LKE </li>
                                        </ol>
                                        Gabung dokumen PDF (<i>merge</i> ) di atas menjadi 1 file PDF. Anda bisa menggunakan tool online berikut untuk menggabung (<i>merge</i> ) file PDF <a href="https://www.ilovepdf.com/merge_pdf" target="_blank">di sini</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Upload</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- modal cpar -->
    <div class="modal fade" id="modalCPAR" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="">Form CPAR</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form class="form-cpar" data-id="">
                    <div class="modal-body">
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Jenis</label>
                            <select name="jenis" class="form-control" disabled="">
                                <option value="Asesmen Internal">Asesmen Internal</option>
                                <option value="Asesmen External">Asesmen External</option>
                                <option value="Output Tidak Sesuai">Output Tidak Sesuai</option>
                                <option value="Keluhan Pelanggan">Keluhan Pelanggan</option>
                                <option value="Kinerja Penyedia Eksternal">Kinerja Penyedia Eksternal</option>
                                <option value="Sasaran Mutu">Sasaran Mutu</option>
                                <option value="Temuan Internal">Temuan Internal</option>
                                <option value="Dokumen SAPM">Dokumen SAPM</option>
                                <option value="Lain-lain">Lain-lain</option>
                            </select>
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Ketidaksesuaian</label>
                            <textarea name="ketidaksesuaian" id="" cols="30" rows="5" class="form-control" disabled=""></textarea>
                        </div>
                        <hr>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Koreksi yang dilakukan</label>
                            <textarea name="koreksi" id="" cols="30" rows="5" class="form-control"></textarea>
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Penyebab Ketidaksesuaian</label>
                            <textarea name="penyebab" id="" cols="30" rows="5" class="form-control"></textarea>
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Usulan Tindakan korektif yang dilaksanakan</label>
                            <textarea name="usulan" id="" cols="30" rows="5" class="form-control"></textarea>
                        </div>
                        <div class="position-relative form-group"><label for="exampleSelect" class="">Verifikasi Tindakan Korektif</label>
                            <select name="verifikasi" class="form-control">
                                <option value="Efektif">Efektif</option>
                                <option value="Tidak Efektif">Tidak Efektif</option>
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-primary">Simpan</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <!-- modal untuk segala pesan confirmasi -->
    <div class="modal fade" id="modal_confirm" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title modal-confirm-message">Apakah Anda Yakin ?</h5>
                    <!-- <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button> -->
                </div>
                <form class="form-confirm" data-id="">
                    <div class="modal-body modal-confirm-body"></div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-danger modal-confirm-btnNo" data-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-primary modal-confirm-btnYes">Ya</button>
                    </div>
                </form>
            </div>
        </div>
    </div>

    <div data-v-e9185a48="">
        <!---->
    </div>
    <div class="jvectormap-tip"></div>
</body>

</html>